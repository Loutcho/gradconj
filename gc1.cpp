...
Méthode du gradient conjugué préconditionné par SSOR
...
const float omega=1.93966925705; // omega=2/(1+2*sin(pi*h/2))
...
public:
   ...
   // Résolution de Cg=r, autrement dit, un pas SSOR de Ag=r
   friend vecteur invC(vecteur);
...
vecteur invC(vecteur d)
{
   ...
   {
   	somme=0;
      if (i-n >= 0) { somme += -y.tab[i-n]; };
      if (i%n != 0) { somme += -y.tab[i-1]; };
      y.tab[i] = (omega/4.0)*(d.tab[i]-somme);
   }
   for (i=N-1;i>=0;i--)
   {
   	somme=0;
      if (i+n  <  N ) { somme += -yy.tab[i+n]; };
      if (i%n != n-1) { somme += -yy.tab[i+1]; };
      yy.tab[i] = (2.0-omega)*y.tab[i] - (omega/4.0)*somme;
   }
   return yy;
}
...
{
	FILE *stream;
   stream = fopen("c:\\windows\\bureau\\gc1.txt","w+");

	vecteur b,x,r,p,z,ff,g;
